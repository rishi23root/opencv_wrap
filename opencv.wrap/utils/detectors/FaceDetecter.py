...
class Face:
    ...
    def __init__(self):
        self.trained_model = cv2.CascadeClassifier(
        ...
        for i in self.face_coordinate:
            ...
            if draw:
                cv2.rectangle(
                    ...
                    (x + w + self.padding, y + h + self.padding),
                    (0, 255, 0),
                    2,
                )
            # crop
            roiFrame = frame[
                y - self.padding : self.padding + h + y,
            ...
            frame[
                y - self.padding : self.padding + h + y,
                x - self.padding : self.padding + w + x,
            ] = self.actionOnImage(roiFrame, action="blur")
        return frame
    ...
    def detectFaceAndShow(self, frame, draw=False):
        ...
        for index in range(len(self.face_coordinate)):
            ...
            try:
                frame[fh - self.image_size : fh, fw - self.image_size : fw] = (
                    cv2.resize(roiFrame, (self.image_size, self.image_size))
                )
            except:
                pass
    ...
    def detectFaceAndCenter(self, frame, draw=False, padding=100):
        ...
        if len(self.face_coordinate) >= 1:
            ...
            # crop
            roiFrame = frame[
                y - padding : padding + h + y, x - padding : padding + w + x
            ]
            # # put back on
            try:
